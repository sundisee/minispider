...
cur=conn.cursor()
cur.execute('select poi_id from poinfo where poi_id IS NOT NULL  and poi_type = "娱乐" and poi_fen IS NULL  order by id')
result = cur.fetchall()
...
class ProvinceSpider(BaseSpider):
    name = "poi_yule_content"
    allowed_domains = ["www.mafengwo.cn"]
    start_urls = jingdian_poi_urls

    def parse(self, response):
        hxs = HtmlXPathSelector(response)
        lat = re.compile(r'lat : parseFloat\(\"(-{0,1}\d+.{0,1}\d{0,10})\"\)',re.S).search(response.body)
        lng = re.compile(r'lng : parseFloat\(\"(-{0,1}\d+.{0,1}\d{0,10})\"\)',re.S).search(response.body)
